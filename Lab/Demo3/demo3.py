...
class SimpleMonitor13(simple_switch_13.SimpleSwitch13):
    ...
    def delete_flow(self, datapath, priority, match):
        ...
        mod = parser.OFPFlowMod(datapath=datapath, command=ofproto.OFPFC_DELETE,
                                priority=priority, match=match, out_port=ofproto.OFPP_ANY, out_group=ofproto.OFPG_ANY)
    ...
    def _flow_stats_reply_handler(self, ev):
        ...
        for stat in sorted([flow for flow in body if flow.priority == 1],
            ...
            if(stat.packet_count >= self.drop_threshold):
                self.logger.info(
                    f"Remove switch={sw_id}  pkt_count={pkt_count}  in_port={in_port}  eth_src={eth_src} eth_dst={eth_dst}")
